...
// @version      3.3
// @description  V3.3: 增加单条/批量删除记录功能。V3.2: 修复V3.1中预算采集抓成系列名称的BUG。
// @author       Taoo (V3.3 by Gemini)
...
(function() {
    ...
    function addControls() {
        ...
        GM_addStyle(`
            ...
            /* V3.3: 新增删除按钮的样式 */
            .tt-helper-btn-delete-selected { background-color: #c9302c; margin-left: 10px; }
        `);
        ...
        panel.innerHTML = `
            ...
            <div id="tt-helper-results">
                <table id="tt-panel-table">
                    <thead>
                        <tr>
                            <th style="width: 20px;"><input type="checkbox" id="tt-panel-select-all" title="全选/全不选"></th>
            ...
            <div class="tt-helper-controls">
                <span>
                    ...
                    <button class="tt-helper-btn tt-helper-btn-delete-selected" id="tt-helper-delete-selected" title="删除选中的行">删除所选</button>
                </span>
        ...
        // V3.3: 绑定新按钮事件
        panel.querySelector('#tt-helper-delete-selected').addEventListener('click', deleteSelectedData);
        panel.querySelector('#tt-panel-select-all').addEventListener('click', toggleSelectAll);
    ...
    function showSearchPanel() {
        ...
        // V3.3: 重置全选复选框
        const selectAllBox = panel.querySelector('#tt-panel-select-all');
    ...
    function renderPanelTable(data) {
        ...
        // V3.3: 渲染前取消全选
        const selectAllBox = document.getElementById('tt-panel-select-all');
        if(selectAllBox) selectAllBox.checked = false;
        ...
        if (filteredData.length === 0) {
            // V3.3: Colspan 7 (增加了复选框列)
            tbody.innerHTML = '<tr><td colspan="7">没有找到数据</td></tr>'; 
            return;
        }
        ...
        filteredData.forEach(r => {
            ...
            // V3.3: 增加复选框列, 并在复选框上存储 account-id 和 campaign-name
            tr.innerHTML = `
                <td>
                    <input type="checkbox" class="tt-panel-row-cb" 
                           data-account-id="${r.id}" 
                           data-campaign-name="${r.campaign}">
                </td>
                <td>${r.bcName || 'N/A'}</td>
    ...
    function toggleSelectAll() {
        try {
            const isChecked = document.getElementById('tt-panel-select-all').checked;
            const rowCheckboxes = document.querySelectorAll('#tt-panel-tbody .tt-panel-row-cb');
            rowCheckboxes.forEach(cb => cb.checked = isChecked);
        } catch (e) {
            console.error('[TT助手] 全选功能出错:', e);
        }
    }

    /**
     * V3.3: 删除所选数据
     * 遍历选中的复选框, 从数据库中删除对应的系列条目
     */
    function deleteSelectedData() {
        const selectedCheckboxes = document.querySelectorAll('#tt-panel-tbody .tt-panel-row-cb:checked');
        ...
        if (confirm(`您确定要删除选中的 ${selectedCheckboxes.length} 条记录吗？\n\n这个操作无法撤销，数据将从本地存储中永久删除。`)) {
            try {
                const db = loadDb();
                let deletedCount = 0;
                ...
                selectedCheckboxes.forEach(cb => {
                    ...
                    const campaignName = cb.dataset.campaignName;

                    // 检查数据是否存在, 然后删除
                    if (db[accountId] && db[accountId].campaigns && db[accountId].campaigns[campaignName]) {
                        
                        delete db[accountId].campaigns[campaignName];
                        deletedCount++;

                        // (可选的清理): 如果这个账户下已经没有任何系列了, 
                        // 并且这个账户也没有BC名称 (意味着它是一个孤立的空账户), 
                        // 我们可以把整个账户也删掉。
                        // 但为了安全起见, 目前的逻辑是只删除系列 (campaign)。
                        // if (Object.keys(db[accountId].campaigns).length === 0 && !db[accountId].bcName) {
                        //     delete db[accountId];
                        // }
                    }
                });
                ...
                saveDb(db); // 保存修改后的数据库
                console.log(`[TT助手] 成功删除 ${deletedCount} 条记录。`);
                
                // 刷新面板以显示最新数据
                showSearchPanel(); 

            } catch (e) {
                console.error('[TT助手] 删除选中数据时出错:', e);
                alert("删除数据时发生错误，请查看控制台。");
            }
        }
    }
